...
// シェーダファイル
static const std::string VERT_SHADER_FILE = std::string(SHADER_DIRECTORY) + "Oren-Nayar.vert";
static const std::string FRAG_SHADER_FILE = std::string(SHADER_DIRECTORY) + "Oren-Nayar.frag";
...
// 物体の速度を格納
glm::vec3 ball_v = glm::vec3(3.0f, 4.0f, 0.0f);
...
void paintGL()
{
    ...
    glm::mat4 viewMat = glm::lookAt(glm::vec3(10.0f, 10.0f, 10.0f), // 視点の位置
    ...
    glm::mat4 mvpMat = projMat * viewMat * transMat * modelMat;
...
void animate()
{
    ...
    ball_v += glm::vec3(0.0f, grav_a * dt, 0.0f);

    // 変位を計算
    glm::vec3 dsp = glm::vec3(ball_v.x * dt, ball_v.y * dt, ball_v.z * dt);
    transMat = glm::translate(transMat, dsp);
}
