...
//背景の正方形は座標変換しないため別のvertファイルを用意
static std::string VERT_SHADER_FILE_BG = std::string(SHADER_DIRECTORY) + "BG_render.vert";
static std::string FRAG_SHADER_FILE_BG = std::string(SHADER_DIRECTORY) + "render.frag";
...
{
    GradationBackground(const glm::vec3 &top, const glm::vec3 &bottom) : top(top), bottom(bottom) {}

    const glm::vec3 top;
    const glm::vec3 bottom;

    glm::vec3 grad_positions[4] = {
        glm::vec3(-1.0f, -1.0f, 0.999f),
        glm::vec3(-1.0f, 1.0f, 0.999f),
        glm::vec3(1.0f, -1.0f, 0.999f),
        glm::vec3(1.0f, 1.0f, 0.999f)};
    glm::vec3 grad_colors[2] = {bottom, top};
    unsigned int grad_indices[6][2] = {{0, 0}, {1, 1}, {2, 0}, {1, 1}, {2, 0}, {3, 1}};
...
void GradationBackground::initBg()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        Vertex v(grad_positions[grad_indices[i][0]], grad_colors[grad_indices[i][1]]);
        ...
        // VAOをOFFにしておく
        glBindVertexArray(0);
        programId = buildShaderProgram(VERT_SHADER_FILE_BG, FRAG_SHADER_FILE_BG);
    };
}

void GradationBackground::drawBg()
{
    // シェーダの有効化
...
int main()
{
    ...
    GradationBackground gbg(glm::vec3{0.0f, 0.0f, 1.0f}, glm::vec3{0.0f, 0.0f, 0.0f});
    Cube cd;
