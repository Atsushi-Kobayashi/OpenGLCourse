...
// Objectを選択する操作を行うか判定するための変数
glm::ivec2 pressPos = glm::ivec2(0, 0);
...
static const unsigned int faces[12][3] = {
    {1, 6, 7}, {1, 7, 4}, {2, 5, 7}, {2, 7, 4}, {3, 5, 7}, {3, 7, 6}, {0, 1, 4}, {0, 4, 2}, {0, 1, 6}, {0, 6, 3}, {0, 2, 5}, {0, 5, 3}};
...
{
    ArcballState() {}
    glm::mat4 acRotMat = glm::mat4(1.0);
    glm::mat4 acTransMat = glm::mat4(1.0);
    glm::mat4 acScaleMat = glm::mat4(1.0);
    ...
    void initState(float init_x);
};

void ArcballState::initState(float init_x)
{
    // model行列の設定
    modelMat = glm::translate(modelMat, glm::vec3(init_x, 0.0f, 0.0f));
    ...
    gravity += glm::vec3(init_x, 0.0f, 0.0f);
}
...
void paintGL()
{
    ...
    // 1つ目の立方体を描画
    {
        glm::mat4 mvpMat = projMat * viewMat * ABS_1.modelMat * ABS_1.acTransMat * ABS_1.acRotMat * ABS_1.acScaleMat;
        // Uniform変数の転送
        GLuint uid;
        ...
        uid = glGetUniformLocation(programId, "u_selectedObjectID");
        glUniform1i(uid, selectedObject);
    ...
    // 2つ目の立方体を描画
    {
        glm::mat4 mvpMat = projMat * viewMat * ABS_2.modelMat * ABS_2.acTransMat * ABS_2.acRotMat * ABS_2.acScaleMat;
        // Uniform変数の転送
        GLuint uid;
        uid = glGetUniformLocation(programId, "u_mvpMat");
        glUniformMatrix4fv(uid, 1, GL_FALSE, glm::value_ptr(mvpMat));
...
void mouseEvent(GLFWwindow *window, int button, int action, int mods)
{
    ...
    if (action == GLFW_PRESS)
    {
        if (!isDragging)
        {
            ...
            pressPos = glm::ivec2(px, py);
        }
    }
    else
    {
        // 以下Objectを選択する処理（移動の操作との衝突を避けるため、クリック時ではなく離した時に選択）
        const int cx = (int)px;
        const int cy = (int)py;

        // このままでは回転操作のあとマウスボタンを離した部分のObjectが選択されてしまうため、
        // press位置とrelease位置が離れている時はObject選択が起きないように
        const double dx = px - pressPos.x;
        const double dy = py - pressPos.y;
...
void updateRotate(ArcballState &ABS)
{
    static const double Pi = 4.0 * std::atan(1.0);
    ...
    const glm::mat4 c2oMat = glm::inverse(viewMat * ABS.modelMat);

    // オブジェクト座標における回転軸
    const glm::vec3 rotAxisObjSpace = glm::vec3(c2oMat * glm::vec4(rotAxis, 0.0f));

    // 回転行列の更新
    ABS.acRotMat = glm::rotate((float)(4.0 * angle), rotAxisObjSpace) * ABS.acRotMat;
}

void updateTranslate(ArcballState &ABS)
{
    // オブジェクト重心のスクリーン座標を求める
    glm::vec4 gravityScreenSpace = (projMat * viewMat * ABS.modelMat) * glm::vec4(ABS.gravity.x, ABS.gravity.y, ABS.gravity.z, 1.0f);
    ...
    const glm::mat4 s2oMat = glm::inverse(projMat * viewMat * ABS.modelMat);
    ...
    // オブジェクト空間での平行移動
    ABS.acTransMat = glm::translate(ABS.acTransMat, transObjSpace);
}

void updateScale(ArcballState &ABS)
{
    ABS.acScaleMat = glm::scale(glm::vec3(ABS.acScale, ABS.acScale, ABS.acScale));
}

void updateMouse(ArcballState &ABS)
{
    switch (arcballMode)
    {
    ...
    case ARCBALL_MODE_SCALE:
        ABS.acScale += (float)(oldPos.y - newPos.y) / WIN_HEIGHT;
        updateScale(ABS);
        break;
    }
}
...
void wheelEvent(GLFWwindow *window, double xpos, double ypos)
{
    if (selectedObject == 1)
    {
        ABS_1.acScale += ypos / 10.0;
        updateScale(ABS_1);
    }
    else if (selectedObject == 2)
    {
        ABS_2.acScale += ypos / 10.0;
        updateScale(ABS_2);
    }
}
...
int main(int argc, char **argv)
{
    ...
    ABS_1.initState(-2.0f);
    ABS_2.initState(2.0f);
