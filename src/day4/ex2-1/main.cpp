...
// static std::string TEX_FILE = std::string(DATA_DIRECTORY) + "/NBA_BASKETBALL/NBA BASKETBALL DIFFUSE.tga";
...
struct Vertex
{
    // シェーダ側でintはエラーになる（補間できない）ので注意
    Vertex(const glm::vec3 &position_, const glm::vec2 &tex_, const int faceID_)
        : position(position_), tex(tex_), faceID(faceID_)
    {
    }
...
static const glm::vec2 tex_coords[14] = {
...
static const unsigned int indices_tex[12][3] = {
    {12, 13, 11}, {12, 11, 10}, {7, 8, 11}, {7, 11, 10}, {4, 8, 9}, {4, 9, 5}, {3, 2, 6}, {3, 6, 7}, {3, 0, 1}, {3, 1, 4}, {3, 7, 8}, {3, 8, 4}};
...
void initVAO()
{
    ...
    for (int face = 0; face < 6; face++)
    {
        for (int i = 0; i < 3; i++)
        {
            vertices.push_back(Vertex(positions[faces[face * 2 + 0][i]], tex_coords[indices_tex[face * 2 + 0][i]], face + 1));
            indices.push_back(idx++);
        }

        for (int i = 0; i < 3; i++)
        {
            vertices.push_back(Vertex(positions[faces[face * 2 + 1][i]], tex_coords[indices_tex[face * 2 + 1][i]], face + 1));
            indices.push_back(idx++);
        }
    }
...
void paintGL()
{
    ...
    glm::mat4 modelMat = glm::rotate(theta, glm::vec3(1.0f, -1.0f, 0.0f));
    ...
    // selectされてるか
    LocId = glGetUniformLocation(programId, "u_selectID");
    glUniform1i(LocId, selectMode ? 1 : -1);

    // 選択されてる面のfaceID
    LocId = glGetUniformLocation(programId, "selectedFaceID");
    glUniform1i(LocId, selected_face_ID);
