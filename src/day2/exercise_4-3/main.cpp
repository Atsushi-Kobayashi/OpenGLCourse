...
void initializeGL() {
    ...
    glm::vec3 add1(1.0, 0.0, 0.0);
    glm::vec3 add2(0.0, 1.0, 0.0);
    glm::vec3 add3(0.0, 0.0, 1.0);
        ...
        for (int b = 0; b < 10; b++){
            for (int c = 0; c < 10; c++){
                for (int face = 0; face < 6; face++) {
                    glm::vec3 add(3 * a, 3 * b, 3 * c);
                    for (int i = 0; i < 3; i++) {
                        glm::vec3 new_pos1 = positions[faces[face * 2 + 0][i]] + add;
                        vertices.push_back(Vertex(new_pos1, colors[face]));
                        indices.push_back(idx++);
                    }

                    for (int i = 0; i < 3; i++) {
                        glm::vec3 new_pos2 = positions[faces[face * 2 + 1][i]] + add;
                        vertices.push_back(Vertex(new_pos2, colors[face]));
                        indices.push_back(idx++);
                    }
                }
            }
...
void paintGL() {
    ...
    glLoadIdentity();
    gluLookAt(60.0f, 80.0f, 100.0f,     // 視点の位置
...
int main(int argc, char **argv) {
    ...
    while (glfwWindowShouldClose(window) == GL_FALSE) {
        ...
        // 描画
        paintGL();
        time += glfwGetTime() - tmp;

        // 描画用バッファの切り替え
        glfwSwapBuffers(window);
        glfwPollEvents();

        //1000個のキューブ描画を1000回繰り返し時間を計測
        if (count == 1000)
        {    
            std::cout << time <<"[ms] (paintGL関数 1回あたり)\n";
            break;
        }    
        count++;
    }
